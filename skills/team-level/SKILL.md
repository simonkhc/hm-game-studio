name: team-level
description: Coordinate level-designer, world-builder, and technical-artist on level creation.
allowed-tools: read_file, write_file, delegate_task, clarify, search_files
model: sonnet
agent: game-designer

Team Level — Multi-Agent Feature Coordination

Coordinates multiple specialists via delegate_task(tasks=[...]) to implement a level design feature end-to-end.
...
Read all relevant GDDs for the level design systems.
...
- level-designer: designs layout and pacing
- world-builder: ensures level fits world lore
- technical-artist: creates environment assets and lighting
