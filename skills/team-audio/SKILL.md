name: team-audio
description: Coordinate sound-designer and audio-director on audio implementation — SFX, music, ambient, mixing.
allowed-tools: read_file, write_file, delegate_task, clarify, search_files
model: sonnet
agent: audio-director

Team Audio — Multi-Agent Feature Coordination

Coordinates multiple specialists via delegate_task(tasks=[...]) to implement a audio feature end-to-end.
...
Read all relevant GDDs for the audio systems.
...
- sound-designer: creates SFX assets and event lists
- audio-director: reviews for consistency, sets mixing levels
- gameplay-programmer: implements audio triggers in code (if needed)
