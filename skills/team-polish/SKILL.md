name: team-polish
description: Coordinate performance-analyst, technical-artist, and sound-designer on polish pass.
allowed-tools: read_file, write_file, delegate_task, clarify, search_files
model: sonnet
agent: producer

Team Polish — Multi-Agent Feature Coordination

Coordinates multiple specialists via delegate_task(tasks=[...]) to implement a polish feature end-to-end.
...
Read all relevant GDDs for the polish systems.
...
- performance-analyst: profiles and optimizes
- technical-artist: adds visual polish (particles, animations)
- sound-designer: adds audio polish (transition sounds, ambiance)
...
Present:
```
## Team Polish — Integration Report
